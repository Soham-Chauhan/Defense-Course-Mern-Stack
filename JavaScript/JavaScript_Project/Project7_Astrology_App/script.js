...
form.addEventListener('submit', (e)=>{
    ...
    const text = `Hi ${name} ${surname}, your zodic sign is ${zodiacSigns[month-1]}. ${compliments[day-1]}. ${victimCardCompliments[year%20]}. ${recommendations[(day*month)%30]}. ${predictions[(name.length*surname.length)%20]}`;

    const result = document.getElementById('result').textContent = text;
